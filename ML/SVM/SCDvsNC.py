...
# ============================ Data Preparation ============================ #
df = pd.read_csv("SCDvsNC_Feature.csv")
df = shuffle(df, random_state=42)

X = df[['Component_1', 'Component_2', 'Component_3', 'Component_5', 'Component_6',
        'Component_13', 'Component_15', 'Component_16', 'Component_17', 'Component_18']]

y = df['Label'].map({'SCD': 0, 'NC': 1})
y_counts = y.value_counts()
mapped_counts = y_counts.rename(index={0: 'SCD', 1: 'NC'})
print(mapped_counts)
...
skf = StratifiedKFold(n_splits=5, shuffle=True, random_state=163)
for fold, (train_idx, test_idx) in enumerate(skf.split(X, y), 1):
    ...
    mapped_train_counts = y_train_counts.rename(index={0: 'SCD', 1: 'NC'})
    ...
    mapped_test_counts = y_test_counts.rename(index={0: 'SCD', 1: 'NC'})
    ...
    train_fold_report = classification_report(y_train, y_train_pred, target_names=['SCD', 'NC'])
    train_fold_reports.append(train_fold_report)
    test_fold_report = classification_report(y_test, y_test_pred, target_names=['SCD', 'NC'])
