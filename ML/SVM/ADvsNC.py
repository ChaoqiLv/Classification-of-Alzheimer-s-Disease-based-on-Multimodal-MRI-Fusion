...
# ============================ Custom class for dual output to console and file ============================ #
class Logger(object):
...
# Redirect standard output to custom Logger class
sys.stdout = Logger("program_log.txt")

# ============================ Data Preparation ============================ #
df = pd.read_csv("ADvsNC_Feature.csv")
df = shuffle(df, random_state=12)

# Use selected features from feature selection
X = df[['Component_2', 'Component_4', 'Component_5', 'Component_9', 'Component_11',
        'Component_13', 'Component_15', 'Component_16', 'Component_17', 'Component_18']]

y = df['Label'].map({'AD': 0, 'NC': 1})
y_counts = y.value_counts()
mapped_counts = y_counts.rename(index={0: 'AD', 1: 'NC'})
print(mapped_counts)
...
# Add to data storage initialization section
train_confusion_matrices = []  # Store confusion matrices for each training fold
test_confusion_matrices = []   # Store confusion matrices for each test fold
...
# Set uniform random seed
random_seed = 169

# ============================ Stratified Cross-validation Process ============================ #
# Create a StratifiedKFold object for 5-fold stratified cross-validation
...
for fold, (train_idx, test_idx) in enumerate(skf.split(X, y), 1):
    ...
    mapped_train_counts = y_train_counts.rename(index={0: 'AD', 1: 'NC'})
    ...
    mapped_test_counts = y_test_counts.rename(index={0: 'AD', 1: 'NC'})
    ...
    # Training set ROC data collection
    ...
    train_fold_report = classification_report(y_train, y_train_pred, target_names=['AD', 'NC'])
    train_fold_reports.append(train_fold_report)

    test_fold_report = classification_report(y_test, y_test_pred, target_names=['AD', 'NC'])
...
# ============================= Calculate training set average confusion matrix =============================
...
def save_confusion_matrix_data(cm, filename):
    df = pd.DataFrame(cm, index=['AD', 'NC'], columns=['AD', 'NC'])
    df.to_csv(filename)
