...
# ============================ Data Preparation ============================ #
df = pd.read_csv("MCIvsNC_Feature.csv")
df = shuffle(df, random_state=99)

# Select specified columns from the DataFrame as feature matrix X
X = df[['Component_1', 'Component_2', 'Component_3', 'Component_8', 'Component_9',
        'Component_13', 'Component_15', 'Component_16', 'Component_17', 'Component_18']]

y = df['Label'].map({'NC': 1, 'MCI': 0})
y_counts = y.value_counts()
mapped_counts = y_counts.rename(index={1: 'NC', 0: 'MCI'})
print(mapped_counts)
...
for fold, (train_idx, test_idx) in enumerate(skf.split(X, y), 1):
    ...
    mapped_train_counts = y_train_counts.rename(index={1: 'NC', 0: 'MCI'})
    print(mapped_train_counts)

    # Downsampling operation
    much_indices = y_train[y_train == 0].index
    few_indices = y_train[y_train == 1].index
    ...
    mapped_test_counts = y_test_counts.rename(index={1: 'NC', 0: 'MCI'})
    ...
    train_fold_report = classification_report(y_train, y_train_pred, target_names=['MCI', 'NC'])
    train_fold_reports.append(train_fold_report)

    test_fold_report = classification_report(y_test, y_test_pred, target_names=['MCI', 'NC'])
...
def save_confusion_matrix_data(cm, filename):
    df = pd.DataFrame(cm, index=['MCI', 'NC'], columns=['MCI', 'NC'])
    df.to_csv(filename)
