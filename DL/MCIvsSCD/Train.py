...
import random
from model.ResNet import resnet10_3d, resnet18_3d, resnet34_3d, resnet50_3d, resnet101_3d, resnet152_3d, resnet200_3d
import json
import copy
import itertools
import time
...
class EarlyStopping:
    def __init__(self, patience=20, verbose=False, delta=0.0, save_dir='model_result', fold=0):
        ...
        self.best_roc_data = {}  # Added: store best ROC data
...
def train_model(fold, num_epochs, lr, device, save_dir, train_dataloader, val_dataloader, test_dataloader):
    ...
    modality_weights = torch.tensor([0.25, 0.25, 0.25, 0.25], device=device)  # GM, FA, MD, ICs
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for inputs, labels in tqdm(trainval_loaders[phase], desc=f'Epoch {epoch + 1}/{num_epochs} {phase}',
                                       disable=True):
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    # Backward propagation (handles multi-modal gradients automatically)
                    if phase == 'train':
                        total_loss.backward()
                        for optimizer in optimizers:
                            optimizer.step()
            ...
            print(
                f"[{phase}] Epoch {epoch + 1}/{num_epochs} Loss: {epoch_loss:.4f} Acc: {epoch_acc:.4f} MCC: {epoch_mcc:.4f}")
            ...
            if phase == 'val':
                ...
                new_weights = torch.softmax(acc_tensor / 0.1,
                                            dim=0)  # Temperature coefficient 0.1 controls weight differences
    ...
    # ================== Test Evaluation ==================
    print(
        f"Testing with best validation model (epoch {early_stopping.best_epoch + 1}, acc {early_stopping.val_acc_max:.4f})...")
...
if __name__ == "__main__":
    ...
    for fold, (train_val_idx, test_idx) in enumerate(skf.split(X, y)):
        ...
        val_dataset = MultiModalDataset(X_val, y_val, training=False)
    ...
    plot_confusion_matrix(avg_cm, classes=class_names,
                          ...
                          normalize=True)
    plt.savefig(os.path.join(save_dir, 'average_internal_confusion_matrix_normalized.png'), dpi=1200,
                bbox_inches='tight')
    plt.close()
